...
const DEPTH = 2; // TODO: make argument instead of hard coded
...
export class SegmentedWall extends MoldableCubeGeometry {
  ...
  constructor(segmentWidth: number[], segmentHeight: number, topSegments: number[], bottomSegments: number[], startingX = 0,startingY = 0) {
    ...
    super({ width_: segmentWidth[0], height_: topSegments[0], depth: DEPTH, widthSegments: 6, fixedTextureSize: 6, segmentedWallArgs: { isTop: true, wallHeight: segmentHeight, runningLeft }});
    ...
    topSegments.forEach((top, index) => {
      ...
      if (index > 0 && top > 0) {
        this.merge(new MoldableCubeGeometry({ width_: currentWidth, height_: top, depth: DEPTH, widthSegments: 6, fixedTextureSize: 6, segmentedWallArgs: { isTop: true, wallHeight: segmentHeight, runningLeft}}).translate_(startingX + runningSide + (currentWidth / 2), segmentHeight - top / 2).done_());
      }

      if (bottomSegments[index] > 0) {
        this.merge(new MoldableCubeGeometry({ width_: currentWidth, height_: bottomSegments[index], depth: DEPTH, widthSegments: 6, fixedTextureSize: 6, segmentedWallArgs: { wallHeight: segmentHeight, isTop: false, runningLeft }}).translate_(startingX + runningSide + (currentWidth / 2), bottomSegments[index] / 2).done_());
      }
