...
export class SegmentedWall extends MoldableCubeGeometry {
  ...
  constructor(segmentWidth: number[], segmentHeight: number, topSegments: number[], bottomSegments: number[], startingX = 0,startingY = 0) {
    ...
    super(segmentWidth[0], topSegments[0], DEPTH, 6, 1, 1, 6, { isTop: true, wallHeight: segmentHeight, runningLeft });
    ...
    topSegments.forEach((top, index) => {
      ...
      if (index > 0 && top > 0) {
        this.merge(new MoldableCubeGeometry(currentWidth, top, DEPTH,6,1,1,6,{ isTop: true, wallHeight: segmentHeight, runningLeft}).translate_(startingX + runningSide + (currentWidth / 2), segmentHeight - top / 2).done_());
      }

      if (bottomSegments[index] > 0) {
        this.merge(new MoldableCubeGeometry(currentWidth, bottomSegments[index], DEPTH, 6, 1, 1, 6, { wallHeight: segmentHeight, isTop: false, runningLeft }).translate_(startingX + runningSide + (currentWidth / 2), bottomSegments[index] / 2).done_());
      }
